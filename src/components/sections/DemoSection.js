...
const fetcher = async () => {
  const res = await fetch(`${process.env.NEXT_PUBLIC_STRAPI_URL}/api/demo-section?populate=*`, {
    headers: {
...
const DemoSection = () => {
  const { data, error, isLoading } = useSWR('demo-section', fetcher);

  const fallbackContent = fallback.demoSection;
  if (isLoading) return <div>Loading...</div>;

  const content = !error && data?.data ? data.data : fallbackContent;

  return (
    <section id="democontent" className="mb-14 w-full max-w-6xl px-4" aria-labelledby="demo-heading">
      <div className="flex flex-col items-center justify-between overflow-hidden rounded-md border-2 border-[#008ab8] p-6 lg:flex-row lg:items-stretch lg:p-6">
        <div className="flex-col justify-between space-y-6 text-center lg:mx-4 lg:flex lg:w-[45%] lg:p-4 lg:py-10 lg:text-left">
          <h1 id="demo-heading" className="text-[26px] font-bold text-gray-800" aria-label="ELN Demo Heading">
            {content?.title}
          </h1>
          <div className="space-y-4 text-base text-gray-700" aria-label="ELN Demo description">
          ...
          <div className="flex justify-center lg:justify-start">
            <a
              ...
              hover:shadow-lg"
              aria-label="Visit the Chemotion ELN Demo documentation page">
              {content?.buttonText}
            </a>
          </div>
        </div>
        <div
          className="relative mt-8 flex aspect-[3/2] w-full justify-center overflow-hidden rounded-md border-2 border-[#008ab8] px-2 lg:my-2 lg:mr-2 lg:w-1/2 lg:justify-end lg:px-0"
          aria-hidden="true">
          {content?.image?.url ? (
            <Image
